  ...
  weaken (MkNested ns) = MkNested (map wknName ns)
    where
      wknName : (Name, (Maybe Name, List (Var vars), FC -> NameType -> Term vars)) ->
                (Name, (Maybe Name, List (Var (vars :< n)), FC -> NameType -> Term (vars :< n)))
...
data RawImp' : Type -> Type where
     ...
     -- FIXME: This is a hack brought over from Idris 2, but now that case is
     -- in the core, it hopefully won't need to be here for long
