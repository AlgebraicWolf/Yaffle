...
interface Eq ty => Ord ty where
  ...
  compare x y = assert_total $ if x < y then LT else if x == y then EQ else GT
  ...
  total
  max : ty -> ty -> ty
  max x y = if assert_total (x > y) then x else y

  total
  min : ty -> ty -> ty
  min x y = if assert_total (x < y) then x else y
